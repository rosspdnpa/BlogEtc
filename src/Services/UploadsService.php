...
use Imagine\Gd\Image as LegacyImage;
...
class UploadsService
{
	...
	protected function legacyUploadAndResize(Post $new_blog_post = null, $suggested_title, $image_size_details, $photo)
	{
		...
		$photo->store("uploadcache");
		$uploadedRawFile = storage_path('app/uploadcache/'.$photo->hashName());
		...
		//dd($photo->getRealPath(),$image_filename,$destinationPath);
		$image = new Imagine();
		$resizedImage = $image->open($uploadedRawFile);
		...
		// Try and remove the temporarily uploaded image
		Storage::delete('uploadcache/'.$photo->hashName());
